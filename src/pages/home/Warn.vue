<template>
  <Card title="历史报警" class="h-full">
    <div class="flex w-full table-thead text-center">
      ...
      <span class="table-th" :class="mode == 1 ? 'w-90' : 'w-100'">等级</span>
      <span class="table-th w-90">报警值</span>
      <span class="table-th" :class="mode == 1 ? 'w-250' : 'flex-1'">时间</span>
    </div>
    <div class="flex-1">
      <vue3-seamless-scroll :step="0.1" :list="data" class="scroll" :limitScrollNum="limitScrollNum">
        <div class="flex w-full  text-center" :class="{ 'bg-[rgba(255,255,255,0.1)]': index % 2 > 0 }"
          ...
          <span class="table-td" :class="mode == 1 ? 'w-90' : 'w-100'">
          ...
          <span class="table-td" :class="mode == 1 ? 'w-250' : 'flex-1'">{{ item.createdDate }}</span>
        </div>
