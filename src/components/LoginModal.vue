<template>
  <a-modal :getContainer="useModalContainer" :open="open" centered :maskClosable="false" :closable="false" width="35%"
    ...
    <div class="web-font-dd text-size-24 text-center mt-48 mb-32" style="color:#23AF98">{{ title }}</div>
    <a-form ref="formRef" :model="form" autocomplete="off" class="px-24" @finish="onFinish">
      ...
      <div class="flex my-32 justify-center m-auto">
        <a-button type="primary" ghost class="w-200 h-54 text-size-18 rounded-100"
          @click="form.password = ''; $emit('update:open', false)">取消</a-button>
        <a-button type="primary" class="w-200 h-54 text-size-18 rounded-100 ml-24" html-type="submit">登录</a-button>
      </div>
    </a-form>
  </a-modal>
</template>
