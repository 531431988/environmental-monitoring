<template>
  <a-modal :getContainer="useModalContainer" :open="open" centered :maskClosable="false" :closable="false" :footer="null">
    ...
    <a-form ref="formRef" :model="form" autocomplete="off" class="px-24" @finish="onFinish">
      ...
      <a-form-item name="confirmPassword" :rules="[{ required: true, message: '请两次输入新密码' }]">
        <a-input-password v-model:value="form.confirmPassword" class="h-64 rounded-100 px-24" placeholder="请两次输入新密码" />
      </a-form-item>
      <div class="flex mt-64 mb-24 justify-center m-auto">
        <a-button type="primary" ghost class="w-200 h-64 rounded-100 text-size-24 mx-24" @click="form = {}; $emit('update:open', false)">取消</a-button>
