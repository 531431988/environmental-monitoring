  ...
  <a-modal :getContainer="useModalContainer" :open="open" centered :maskClosable="false" :closable="false"
    ...
    <a-form ref="formRef" :model="form" autocomplete="off" class="px-24" @finish="onFinish">
      <a-form-item name="password" :rules="[{ required: true, message: '请输入新密码' }]">
        <a-input-password v-model:value="form.password" class=" rounded-100 px-24" placeholder="请输入新密码" />
      </a-form-item>
      <a-form-item name="confirmPassword" :rules="[{ required: true, message: '请两次输入新密码' }]">
        <a-input-password v-model:value="form.confirmPassword" class=" rounded-100 px-24" placeholder="请两次输入新密码" />
      </a-form-item>
      <div class="flex my-32 justify-center m-auto">
        <a-button type="primary" ghost class="w-200 rounded-100 mr-24"
          @click="form = {}; $emit('update:open', false)">取消</a-button>
        <a-button type="primary" class="w-200  rounded-100 " html-type="submit">确定</a-button>
      </div>
    </a-form>
  </a-modal>
</template>
